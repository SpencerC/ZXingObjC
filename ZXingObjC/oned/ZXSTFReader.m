...
static float ZX_STF_MAX_AVG_VARIANCE = 0.38f;
static float ZX_STF_MAX_INDIVIDUAL_VARIANCE = 0.78f;

static const int ZX_STF_W = 3; // Pixel width of a wide line
static const int ZX_STF_N = 1; // Pixel width of a narrow line

/** Valid STF lengths. Anything longer than the largest value is also allowed. */
const int ZX_STF_DEFAULT_ALLOWED_LENGTHS[] = { 6, 8, 10, 12, 14 };
 ...
 */
const int ZX_STF_PATTERNS_LEN = 10;
const int ZX_STF_PATTERNS[ZX_STF_PATTERNS_LEN][5] = {
  {ZX_STF_N, ZX_STF_N, ZX_STF_W, ZX_STF_W, ZX_STF_N}, // 0
  {ZX_STF_W, ZX_STF_N, ZX_STF_N, ZX_STF_N, ZX_STF_W}, // 1
  {ZX_STF_N, ZX_STF_W, ZX_STF_N, ZX_STF_N, ZX_STF_W}, // 2
  {ZX_STF_W, ZX_STF_W, ZX_STF_N, ZX_STF_N, ZX_STF_N}, // 3
  {ZX_STF_N, ZX_STF_N, ZX_STF_W, ZX_STF_N, ZX_STF_W}, // 4
  {ZX_STF_W, ZX_STF_N, ZX_STF_W, ZX_STF_N, ZX_STF_N}, // 5
  {ZX_STF_N, ZX_STF_W, ZX_STF_W, ZX_STF_N, ZX_STF_N}, // 6
  {ZX_STF_N, ZX_STF_N, ZX_STF_N, ZX_STF_W, ZX_STF_W}, // 7
  {ZX_STF_W, ZX_STF_N, ZX_STF_N, ZX_STF_W, ZX_STF_N}, // 8
  {ZX_STF_N, ZX_STF_W, ZX_STF_N, ZX_STF_W, ZX_STF_N}  // 9
};

@interface ZXSTFReader ()

@property (nonatomic, assign) int narrowLineWidth;

@end

@implementation ZXSTFReader
...
- (ZXResult *)decodeRow:(int)rowNumber row:(ZXBitArray *)row hints:(ZXDecodeHints *)hints error:(NSError **)error {
  ...
  if (allowedLengths == nil) {
    ...
    for (int i = 0; i < sizeof(ZX_STF_DEFAULT_ALLOWED_LENGTHS) / sizeof(int); i++) {
      [temp addObject:@(ZX_STF_DEFAULT_ALLOWED_LENGTHS[i])];
    }
...
- (ZXIntArray *)decodeStart:(ZXBitArray *)row {
  ...
  ZXIntArray *startPattern = [self findGuardPattern:row rowOffset:endStart pattern:ZX_STF_STF_START_PATTERN patternLen:sizeof(ZX_STF_STF_START_PATTERN)/sizeof(int)];
  if (!startPattern) {
    return nil;
  }
...
- (ZXIntArray *)decodeEnd:(ZXBitArray *)row {
  ...
  ZXIntArray *endPattern = [self findGuardPattern:row rowOffset:endStart pattern:ZX_STF_END_PATTERN_REVERSED patternLen:sizeof(ZX_STF_END_PATTERN_REVERSED)/sizeof(int)];
  if (!endPattern) {
    [row reverse];
    return nil;
  }
  ...
  for (int x = rowOffset; x < width; x++) {
    if ([row get:x] ^ isWhite) {
      ...
      if (counterPosition == patternLength - 1) {
        if ([ZXOneDReader patternMatchVariance:counters pattern:pattern maxIndividualVariance:ZX_STF_MAX_INDIVIDUAL_VARIANCE] < ZX_STF_MAX_AVG_VARIANCE) {
...
- (int)decodeDigit:(ZXIntArray *)counters {
  float bestVariance = ZX_STF_MAX_AVG_VARIANCE; // worst variance we'll accept
  int bestMatch = -1;
  int max = ZX_STF_PATTERNS_LEN;
  for (int i = 0; i < max; i++) {
    int pattern[counters.length];
    for (int ind = 0; ind < counters.length; ind++){
      pattern[ind] = ZX_STF_PATTERNS[i][ind];
    }
    float variance = [ZXOneDReader patternMatchVariance:counters pattern:pattern maxIndividualVariance:ZX_STF_MAX_INDIVIDUAL_VARIANCE];
